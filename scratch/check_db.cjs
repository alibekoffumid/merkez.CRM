...
async function checkMessages() {
  const { data: contacts, error: ce } = await supabase.from('integration_contacts').select('*').limit(5);
  console.log('Recent Contacts:', contacts);

  const { data: messages, error: me } = await supabase.from('integration_messages').select('*').order('created_at', { ascending: false }).limit(5);
