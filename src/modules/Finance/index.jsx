    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">{t('finance.subtitle')}</p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/50 border-b border-gray-100 text-xs uppercase text-gray-500 tracking-wider">
                <th className="font-medium p-4">{t('finance.thId')}</th>
                <th className="font-medium p-4">{t('finance.thDate')}</th>
                <th className="font-medium p-4">{t('finance.thDesc')}</th>
                <th className="font-medium p-4">{t('common.status')}</th>
                <th className="font-medium p-4 text-right">{t('common.price')}</th>
              </tr>
            </thead>
                ...
                <tr key={trx.id} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="p-4">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${trx.status === 'Completed' ? 'bg-green-100 text-merkez-green' : 'bg-yellow-100 text-merkez-yellow'}`}>
                      {trx.status === 'Completed' ? t('status.completed') : t('status.pending')}
                    </span>
                  </td>
