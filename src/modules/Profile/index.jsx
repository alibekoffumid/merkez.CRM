...
const Profile = () => {
  ...
  // URL для QR-кода
  const airMouseControllerUrl = `https://saas.digitall.llc/air-mouse.html?session=${airMouseSession}&v=7`;
  ...
  const [airMouseDevice, setAirMouseDevice] = useState(() => {
    return localStorage.getItem('merkez_airmouse_device') || 'phone';
  });

  const changeAirMouseDevice = (dev) => {
    setAirMouseDevice(dev);
    localStorage.setItem('merkez_airmouse_device', dev);
    window.dispatchEvent(new Event('merkez_airmouse_toggled'));
  };
                             ...
                             {airMouseDevice === 'phone' ? (
                               <div className="flex flex-col md:flex-row items-center gap-8">
                                 ...
                                 <div className="flex-1 space-y-4">
                                   ...
                                   {[
                                     ...
                                     { step: '3', icon: '✋', text: 'Подними руку — курсор на экране начнёт двигаться' },
                               ...
                               <div className="space-y-4">
                                 <h4 className="text-lg font-black text-gray-900 flex items-center gap-2">
                                   <span className="text-2xl">💻</span> Работает через веб-камеру
                                 </h4>
                                 <p className="text-sm font-bold text-gray-600 leading-relaxed max-w-md">
                                   Теперь контроллер встроен прямо в CRM. Разрешите браузеру доступ к камере. 
